...
def get_graph_template(new_graph: nx.Graph, edge_attrs: bool = False) -> Data:
...
class WDNDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        assert removal in ["keep_list", "reservoir", "tank", "keep_junction", "keep_all"], (
        ...
        if do_scale:
            ...
            if edge_attrs:
                ...
                for i in range(len(self._templates)):
                    self._templates[i].edge_attr = scale(
                        self._templates[i].edge_attr,
                        norm_type=norm_type,
                        mean=self.edge_mean,
                        std=self.edge_std,
                        min=self.edge_min,
    ...
    def collect(
        self, input_path: str, zip_file_path: str, feature: str, edge_attrs: list, removal: str, from_set: str, num_records: int, **kwargs
...
class NoisyWDNDataset(WDNDataset):
    ...
    def collect(
        self, input_path: str, zip_file_path: str, feature: str, edge_attrs: list, removal: str, from_set: str, num_records: int, **kwargs
    ) -> tuple[zarr.Array, Data, np.ndarray, list[str]]:
        # addition params
        sim_engine = kwargs.pop("sim_engine", "epynet")
        mean_dmd = kwargs.pop("mean_dmd", 0.0)
        std_dmd = kwargs.pop("sim_engine", 0.0)
        ################
        # root = zarr.open(store=zip_file_path, mode='r')
